...
Once you have opened the `aws-do-ray` shell you will be dropped in the [`/ray`](/Container-Root/ray/) directory where you will find the [`./deploy-kuberay-operator.sh`](/Container-Root/ray/deploy-kuberay-operator.sh) script. This deployment creates a kuberay-operator pod in your EKS cluster within your head node. Upon successful deployment, it will be in Running state. To check the state of the pod in the cluster, use command: `kubectl get pods`. 
...
- [`./create-cluster.sh`](/Container-Root/ray/RayCluster/create-cluster.sh) : this script creates the ray cluster specified in the [`raycluster-template.yaml`](/Container-Root/ray/RayCluster/raycluster-template.yaml) file. 
- [`./delete-cluster.sh`](/Container-Root/ray/RayCluster/delete-cluster.sh) : this script deletes the ray cluster specified in the [`raycluster-template.yaml`](/Container-Root/ray/RayCluster/raycluster-template.yaml) file. 
- [`./ray-expose.sh`](/Container-Root/ray/RayCluster/ray-expose.sh) : exposes and port-forwards the ray dashboard to http://localhost:8265 from the ray head pod of the ray cluster. 
- [`./ray-hide.sh`](/Container-Root/ray/RayCluster/ray-hide.sh) : deletes the port-forward of the ray dashboard to http://localhost:8265 from the ray head pod of the ray cluster. 
...
- [`./submit-job.sh`](/Container-Root/ray/RayCluster/submit-job.sh) : this script allows you to submit a Python Script for a job. You can put your code within the [`/jobs`](/Container-Root/ray/RayCluster/jobs/) section of the repo with a directory named after the script you want to execute, with that script within that directory. Or you can submit it via file system that has your script that is attached to your ray pods. 
	- If your script is in the [`/jobs`](/Container-Root/ray/RayCluster/jobs/) folder, it will submit the ray job via the ray job submission SDK (dashboard must be exposed via [`./ray-expose.sh`](/Container-Root/ray/RayCluster/ray-expose.sh)) or it will submit directly through the head pod. Just run `./submit-job.sh <script name>`. Ex/ `./submit-job.sh dt-pytorch`.
	- If your script is in a file system that is attached to your ray pods, it you must specify the directory that the script is in relative to your head pod. Run `./submit-job.sh <script name> <directory>`. Ex/ `./submit-job.sh dt-pytorch s3/code/dt-pytorch` where i have my dt-pytorch.py file in the directory s3/code/dt-pytorch. 

### RayCluster Template
For everything you need to know about the details of a RayCluster configuration, please refer to the comments in the template, as well as this [doc](https://docs.ray.io/en/latest/cluster/kubernetes/user-guides/config.html)
### Ray Dashboard
...
For an easy way to expose the Kubeflow Dashboard, we can use kubectl port-forward. To start the port-forward, execute script [`./ray-expose.sh`](/Container-Root/ray/RayCluster/ray-expose.sh). 
...
- [`./create-rayjob.sh`](/Container-Root/ray/RayJob/create-rayjob.sh) : this script creates the rayjob specified in the [`rayjob-template.yaml`](/Container-Root/ray/RayJob/rayjob-template.yaml). This consists of a RayJob and a RayCluster. The RayJob manages the RayCluster. 
- [`./delete-rayjob.sh`](/Container-Root/ray/RayJob/delete-rayjob.sh) : this script deletes the rayjob speficied in the [`rayjob-template.yaml`](/Container-Root/ray/RayJob/rayjob-template.yaml)
- [`./ray-expose.sh`](/Container-Root/ray/RayJob/ray-expose.sh) : exposes and port-forwards the ray dashboard to http://localhost:8265 from the ray head pod of the ray cluster. 
- [`./ray-hide.sh`](/Container-Root/ray/RayJob/ray-hide.sh) : deletes the port-forward of the ray dashboard to http://localhost:8265 from the ray head pod of the ray cluster.
...
## Create a RayServe
Within the [`/ray`](/Container-Root/ray/) directory, you will find the [`/RayServe`](/Container-Root/ray/RayServe/) directory. Within this directory, you will find these scripts:
- [`create-serve.sh`](/Container-Root/ray/RayServe/create-serve.sh): this will create the rayservice specified in the [`serve-template.yaml`](/Container-Root/ray/RayServe/serve-template.yaml). This consists of a RayService and a RayCluster. The RayService manages the RayCluster. 
- `./delete-serve.sh`: this will delete the rayservice specified in the [`serve-template.yaml`](/Container-Root/ray/RayServe/serve-template.yaml). 
- [`./expose-serve.sh`](/Container-Root/ray/RayServe/expose-serve.sh) : This ray service can be queried directly from inside the cluster, but to access it from your laptop you need to use port forwarding. To do this, run this command whichport forwards to localhost:8000. 
- [`./hide-serve.sh`](/Container-Root/ray/RayServe/hide-serve.sh) : This stops the port forwarding for querying. 
- [`serve-template.yaml`](/Container-Root/ray/RayServe/serve-template.yaml) : a rayservice configuration for the example trained image classification model. This also creates a raycluster that is managed by the rayservice. 
- [`/serves`](/Container-Root/ray/RayServe/serves/): contains example python serving code for the image classification model. this is the code zipped into `working_dir: s3://eks-ray-bucket/code/serving-scripts/code16.zip` in the serveConfigV2 of the serve template. 
...
[`./deploy-s3-mountpoint.sh`](/Container-Root/ray/deploy/s3-mountpoint/deploy-s3-mountpoint.sh): creates a PV and a PVC for your S3 mount point which you can use in your ray cluster template. 
